...
def translation(input_file, output_file, src, tgt):
    ...
    if not os.path.exists(output_file):
        ...
        sentences_src = [line.strip() for line in open(input_file, encoding="utf8").readlines()]
...
def main(src='zh',
         tgt='en',
         worker=10,
         input_dir="/home/xuancong/airflow/data/stage3",
         ):
    ...
    with ThreadPoolExecutor(max_workers=worker) as pool:
        ...
        for rootdir, dirs, files in os.walk(input_dir):
            ...
            for file in files:
                ...
                output_file = os.path.join(rootdir.replace("stage3", "stage4"), file.replace('.sent', '.i2r.'+src+'2'+tgt))

...
class th_ch3plus_Spider(scrapy.Spider):

    name = 'th_ch3plus'
    allowed_domains = ['ch3plus.com']
    start_urls = ['https://ch3plus.com/news/category/{}'.format(str(id)) for id in range(118534, 358685)]
    translation = {"ม.ค.": 1,
                   "ก.พ.": 2,
                   "มี.ค.": 3,
                   "เม.ย.": 4,
    ...
    def parse(self, response):
        ...
        date_time_str = response.xpath(
            '//div[@class="article-content"]//p[@class="time"]/text()').get()
        ...
        title = response.xpath(
            '//div[@class="article-content"]/h1/text()').get()

        text_nodes = response.xpath(
            '//div[@class="article-content"]/div[@class="text-read article-news"]/p')
...
def main():
    ...
    process.crawl(th_ch3plus_Spider)
    process.start()

...
class TheGuardianSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for day in days:
            date = day.xpath('./@data-id').get()
            date = datetime.strptime(date, "%d %B %Y").replace(tzinfo=timezone(timedelta(hours=8)))
            article_data = {"date": str(date.date())}
            if date < self.start_date:
                return
            if date >= self.end_date:
                continue
            article_urls = days.xpath('.//a[@class="fc-item__link"]/@href').getall()
    ...
    def parse_article(self, response):
        ...
        article_data["text"] = ''
        texts = response.xpath('//div[contains(@class, "article-body-commercial-selector")]/p')
...
def main(output_path='theguardian.jl', start_date=(date.today() - timedelta(1)), end_date=date.today()):

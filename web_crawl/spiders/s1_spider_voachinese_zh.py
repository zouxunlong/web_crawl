...
class VOAChineseCnSpider(scrapy.Spider):
    name = 'voachinese_cn'
    allowed_domains = ['voachinese.com']
    start_urls = ['https://www.voachinese.com/z/1740']
    base_url = 'https://www.voachinese.com/z/1740?p='
    page = 1
    ...
    def parse(self, response):
        ...
        for article in articles:
            ...
            date = datetime.strptime(date, "%Y年%m月%d日").replace(tzinfo=timezone(timedelta(hours=8)))
    ...
    def parse_article(self, response):
        ...
        texts = response.xpath('//div[@class="wsw"]/p')
        for text in texts:
...
def main(output_path='voachinese_zh.jl', start_date=(date.today() - timedelta(1)), end_date=date.today()):
    ...
    process.crawl(VOAChineseCnSpider, start_date=start_date, end_date=end_date)

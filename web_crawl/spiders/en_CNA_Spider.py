...
class en_CNA_Spider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for item in data["result"]:
            ...
            date_time = datetime.strptime(item["release_date"], "%Y-%m-%dT%H:%M:%S%z").replace(tzinfo=None)
    ...
    def parse_article(self, response, *args, **kwargs):
        ...
        texts=[''.join(text_node.xpath(".//text()").getall()).replace(u'\xa0', " ").replace(u'\u3000', " ") for text_node in text_nodes]
        text = "\n".join(texts[:])
        if text:
            yield {"date": date,
                   "title": title,
